...
type Props = {
  form: { prompt: string; exampleTweets: string[]; photo: File | null };
  onChange: (newForm: {
    prompt: string;
...
export function UploadTweetForm({
  form,
  onChange,
  onSubmit,
  loading,
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) onChange({ ...form, photo: file });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {loading ? "Generating..." : "Generate Tweet with Photo"}
      </Button>
      ...
      {result && (
        <div className="p-4 bg-gray-100 border border-gray-300 rounded-md space-y-3 mt-4">
          {result.tweet_text && (
            <div>
